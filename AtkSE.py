...
class BaseMeta(Module):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, device, with_bias=False, lr=0.01, with_relu=False):
        ...
        self.gcn = GCN_AtkSE(nfeat=nfeat,
                       nhid=hidden_sizes[0],
                       nclass=nclass,
                       dropout=0.5,
                       with_relu=False)
...
class AtkSE(BaseMeta):
    ...
    def forward(self, features, ori_adj, labels, idx_train, idx_unlabeled, perturbations):
        ...
        #for i in tqdm(range(2), desc="Perturbing graph"):
            iters = iters+1
            ...
            adj_changes_symm = adj_changes_symm.to(ori_adj.device)
            #print(adj_changes_symm.device)
            #print(ori_adj.device)
            modified_adj = adj_changes_symm + ori_adj
            #print(modified_adj.device)
            ...
            #print(smoothed_grad.device)
            smoothed_grad = smoothed_grad.to(ori_adj.device)
            ...
            for index in range(idxes.shape[0]):
                ...
                if len(wait_list) == self.wait_list:
                    ...
                    for grad_weight in grad_list:
                        ...
                        adj_grad = adj_grad.to(modified_adj.device)
            ...
            #print(modified_adj.device)
            modified_adj =  modified_adj.to(self.adj_changes.device)
            self.adj_changes.data[row_idx][col_idx] += (-2 * modified_adj[row_idx][col_idx] + 1)
            self.adj_changes.data[col_idx][row_idx] += (-2 * modified_adj[row_idx][col_idx] + 1)
            adj_cpu = ori_adj.to(self.adj_changes.device)
            result = self.adj_changes + adj_cpu
