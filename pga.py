...
def select_attacking_targets(logits, labels, idx_attack, adj_t, policy, pre_ratio, select_ratio):
    ...
    degrees = torch.count_nonzero(adj_t, dim=1)[index_attack]
    ...
    # 确保 margins 和 margin_threshold 都在相同的设备上
    margin_threshold = margin_threshold.to(margins.device)
    # 使用 torch.gt 生成由 0 和 1 组成的掩码张量
    pre_mask = torch.gt(margins, margin_threshold) 
    ...
    for item in indicators:
        ...
        # 假设 item_selected 和 selected_idx 都是 PyTorch 张量
        # 如果它们在 GPU 上，先将它们移动到 CPU
        if item_selected.is_cuda:
            item_selected = item_selected.cpu()
...
class PGA(AttackABC):
    ...
    def __init__(self,
                 ...
                 model, device, **kwargs):
        super(PGA, self).__init__(attack_config, pyg_data, model, device)
    ...
    def _construct_modified_edges(self):
        ...
        neighbors = torch.unique(torch.flatten(self.adj[self.attacking_targets]))
        # 如果需要，将邻居节点转换为长整型
        neighbors = neighbors.long().to('cpu')
