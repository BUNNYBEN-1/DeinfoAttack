    ...
    def attack(self, ori_features, ori_adj, labels, idx_train, idx_unlabeled, n_perturbations, ll_constraint=True, ll_cutoff=0.004):
        ...
        for i in tqdm(range(n_perturbations), desc="Perturbing graph"):
            ...
            if adj_meta_score.max() >= feature_meta_score.max():
                ...
                row_idx, col_idx = utils.unravel_index(adj_meta_argmax.long(), ori_adj.shape)
                self.adj_changes.data[row_idx.long()][col_idx.long()] += (-2 * modified_adj[row_idx.long()][col_idx.long()] + 1)
                if self.undirected:
                    self.adj_changes.data[col_idx.long()][row_idx.long()] += (-2 * modified_adj[row_idx.long()][col_idx.long()] + 1)
            else:
                feature_meta_argmax = torch.argmax(feature_meta_score)
                row_idx, col_idx = utils.unravel_index(feature_meta_argmax.long(), ori_features.shape)
                self.feature_changes.data[row_idx.long()][col_idx.long()] += (-2 * modified_features[row_idx.long()][col_idx.long()] + 1)
